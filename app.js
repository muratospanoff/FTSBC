...
document.getElementById('checkout-form').addEventListener('submit', (e) => {
  ...
  if (state.buyerType === 'self') {
    ...
    if (!check.ok) {
      haptic('error');
      // Сообщение по реальной причине отказа — раньше всегда писали
      // «введите 12 цифр», даже если цифр было ровно 12, но не совпадал
      // формат даты рождения/века, что вводило в заблуждение.
      const reasons = {
        format: 'ИИН должен состоять ровно из 12 цифр (без пробелов и букв).',
        century: 'Некорректная 7-я цифра ИИН (отвечает за век/пол).',
        date: 'Первые 6 цифр ИИН (дата рождения, формат ГГММДД) не образуют существующую дату.',
        implausible: 'По указанному ИИН не удалось определить правдоподобный возраст.',
      };
      const msg = 'ИИН указан неверно. ' + (reasons[check.reason] || 'Проверьте правильность ввода.');
      if (inTelegram && tg.showAlert) tg.showAlert(msg); else alert(msg);
      return;
    }
    if (check.age < 21) {

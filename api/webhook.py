...
def format_order(order):
    ...
    if customer.get("deliveryMethod") == "pickup":
        lines.append(f"🏬 Самовывоз: {customer.get('deliveryAddress', '—')}")
    else:
        lines.append(f"🚚 Доставка (бесплатно): {customer.get('deliveryAddress', '—')}")
